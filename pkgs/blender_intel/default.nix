...
stdenv'.mkDerivation (finalAttrs: {
  ...
  postInstall =
    ...
    + lib.optionalString stdenv.hostPlatform.isLinux ''
      mv $out/bin/blender $out/bin/blender_intel
      mv $out/bin/.blender-wrapped $out/bin/.blender_intel-wrapped
      sed -i 's|/\.blender-wrapped|/\.blender_intel-wrapped|g' $out/bin/blender_intel
      sed -e 's|^Exec=blender|Exec=blender_intel|' \
          -e 's|^Name=Blender$|Name=Blender (Intel)|' \
          -i $out/share/applications/blender.desktop
      mv $out/share/applications/blender.desktop $out/share/applications/blender_intel.desktop
    '';
  ...
  postFixup =
    lib.optionalString cudaSupport ''
      for program in $out/bin/blender_intel $out/bin/.blender_intel-wrapped; do
        addDriverRunpath "$program"
      done
    ''
    + lib.optionalString stdenv.hostPlatform.isDarwin ''
      makeWrapper $out/Applications/Blender.app/Contents/MacOS/Blender $out/bin/blender_intel
    '';

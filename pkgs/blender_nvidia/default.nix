...
stdenv'.mkDerivation (finalAttrs: {
  ...
  postInstall =
    ...
    + lib.optionalString stdenv.hostPlatform.isLinux ''
      mv $out/bin/blender $out/bin/blender_nvidia
      mv $out/bin/.blender-wrapped $out/bin/.blender_nvidia-wrapped
      sed -i 's|/\.blender-wrapped|/\.blender_nvidia-wrapped|g' $out/bin/blender_nvidia
      sed -e 's|^Exec=blender|Exec=blender_nvidia|' \
          -e 's|^Name=Blender$|Name=Blender (NVIDIA)|' \
          -i $out/share/applications/blender.desktop
      mv $out/share/applications/blender.desktop $out/share/applications/blender_nvidia.desktop
    '';
  ...
  postFixup =
    lib.optionalString cudaSupport ''
      for program in $out/bin/blender_nvidia $out/bin/.blender_nvidia-wrapped; do
        addDriverRunpath "$program"
      done
    ''
    + lib.optionalString stdenv.hostPlatform.isDarwin ''
      makeWrapper $out/Applications/Blender.app/Contents/MacOS/Blender $out/bin/blender_nvidia
    '';
